...
function Log() {
  ...
  for (var i = 0; i < arguments.length; i++) {
    ...
    if (typeof e == "object" && "r" in e && "g" in e && "b" in e) {
      ...
      txt.text(e);
      first = false;
      txt.css("color", "rgb(" + col.r + ", " + col.g + ", " + col.b + ")");
...
function OnImagesLoaded(res) {
  ...
  function ImageLoadLoop() {
    ...
    var src = rndimage[0];
    if (!new RegExp("https?://", "i").test(src)) {
      src = "http://" + src;
    }
...
function DoLoadingBar() {
  ...
  var worst =
    stats[
      Object.keys(stats).reduce(function(a, b) {
  ...
  $.each(stats, function(elemid, time_taken) {
    ...
    a.innerHTML =
      '<span style="position: relative;top: 1em;">' + a.innerHTML + "</span>";
  });
...
function LoadPlayerList() {
  ...
  var playersurl =
    "http://r6cf.metastruct.uk.to:2095/app/playerlist?server=" + serverid;
  ...
  var OnPlayerList = function(data, textStatus, request) {
    ...
    var same_instance =
      !data.started || data.started == last_server || last_server == 0;
...
(function() {
  ...
  iframe.src =
    "asset://garrysmod/data/loading_screen_data/metastruct_joinspeed2.txt";
